...
void SetupChainParamsBaseOptions(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-newcoin", "Use the hongbao chain. Equivalent to -chain=hongbao. This is your custom cryptocurrency.", ArgsManager::ALLOW_ANY, OptionsCategory::CHAINPARAMS);  // Hongbao coin option
...
std::unique_ptr<CBaseChainParams> CreateBaseChainParams(const ChainType chain)
{
    ...
    case ChainType::NEWCOIN:
        return std::make_unique<CBaseChainParams>("hongbao", 18887);  // Hongbao coin with custom RPC port
    }
    assert(false);
}
